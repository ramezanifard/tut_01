...
if __name__ == "__main__":
    ...
    with MeComSerial("COM5") as mc:
        ...
        success = mc.set_parameter(value=40.0, parameter_id=3000)        
        print(success)
        success = mc.get_parameter(parameter_id=3000)
        print('-->', success)

        ### set/get status: 2010 
        ###  0:Static OFF, 1:Static ON, 2:Live OFF/ON  3:HW Enable
        success = mc.set_parameter(value=0, parameter_id=2010)        
        print(success)

        ### device status: 104
        ###  0:init, 1:Ready, 2:Run  3:Error 4: Bootloader 5:Device will Reset within next 200ms
        object_temp = mc.get_parameter(parameter_id=104)        
        print('Device Status:', object_temp)

        ## read object temperature: 1000
        object_temp = mc.get_parameter(parameter_id=1000)        
        print('object temperature-->', object_temp)
        # print("leaving with-statement, connection will be closed")
        ## read object temperature: 1000
        # success = mc.set_parameter(value=1, parameter_id=6005)        
        # print(success)
        object_temp = mc.get_parameter(parameter_id=102)        
        print(' -->', object_temp)
